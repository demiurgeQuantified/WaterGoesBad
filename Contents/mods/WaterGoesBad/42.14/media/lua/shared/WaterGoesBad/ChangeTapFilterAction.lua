...
---@class ChangeTapFilterAction : ISBaseTimedAction, umbrella.NetworkedTimedAction
---@field modData table
...
function ChangeTapFilterAction:complete()
	...
	if self.isAddFilter then
		local item = inventory:RemoveOneOf(Registries.items.TAP_FILTER, false)
		sendRemoveItemFromContainer(inventory, item)
	else
		local item = inventory:AddItem(Registries.items.TAP_FILTER)
